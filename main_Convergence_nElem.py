...
for nElem in nElems:
    ...
    ax[0].plot(rnodes,U_g_0, marker = 'o', label = 'Elements:{}'.format(nElem))
    ax[0].set(xlabel = 'r [$mm$]', ylabel = '$U_r$ [$mm$]', title= 'Nodal Displacements') 
    ax[0].legend()

    ax[1].plot(gauss_loc, global_sigma[-1,:,0], marker = 'o', label = 'Elements:{}'.format(nElem))
    ax[1].set(xlabel = 'r [$mm$]', ylabel = '$\sigma_{rr}$ [$MPa$]', title= 'Radial Stress at gauss points')
    ax[1].legend()

    fig.subplots_adjust(wspace=0.3)
    fig.suptitle("Convergence Results (Full Load) in Elastic Regime")


# # Analytical Solution
rr = np.linspace(rnodes[0],rnodes[-1],50)
disp_analytical = (rnodes[0]**3*E_v)/(3*rr**2)
sigma_rr_analytical = (-2*youngs_modulus*E_v*rnodes[0]**3)/(3*(1+poissons_ratio)*rr**3)
ax[0].plot(rr,disp_analytical, label = 'Analytical')
ax[1].plot(rr, sigma_rr_analytical, label = 'Analytical') 
ax[0].legend()
ax[1].legend()
